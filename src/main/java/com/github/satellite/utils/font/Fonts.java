package com.github.satellite.utils.font;

import com.github.satellite.utils.MCUtil;
...
public class Fonts implements MCUtil {
    ...
    private static Font getFontTTF(String name, int size) {
        Font font;
        try {
            InputStream is = mc.getResourceManager().getResource(new ResourceLocation("font/" + name + ".ttf")).getInputStream();
    ...
    private static Font getFontOTF(String name, int size) {
        Font font;
        try {
            InputStream is = mc.getResourceManager().getResource(new ResourceLocation("font/" + name + ".otf")).getInputStream();
            font = Font.createFont(0, is);
            font = font.deriveFont(0, size);
        } catch (Exception ex) {
            ex.printStackTrace();
